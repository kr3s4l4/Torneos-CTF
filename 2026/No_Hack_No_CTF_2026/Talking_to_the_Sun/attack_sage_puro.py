...
# Parámetros CORRECTOS de BrainpoolP512r1
# https://datatracker.ietf.org/doc/html/rfc5639#section-3.8
...
# Orden del generador (n)
# De la API: 8948962207650232551656602815159153422162609644098354511344597187200057010413418528378981730643524959857451398370029280583094215613882043973354392115544169
...
# Verificar orden
n = G.order()
print(f"[+] Orden del generador: {n}")
print(f"[+] Orden esperado: {ORDER}")
print(f"[+] Coinciden: {n == ORDER}")
...
for idx, row in enumerate(L):
    ...
    if all(k < B_non * 100 for k in k_vals) and any(k != 0 for k in k_vals):
        ...
        for sign in [1, -1]:
            ...
            if valid:
                print(f"[+] CLAVE PRIVADA: {hex(d_test)}")
                ...
                with open('/work/admin_token.txt', 'w') as f:
                    f.write(token)
