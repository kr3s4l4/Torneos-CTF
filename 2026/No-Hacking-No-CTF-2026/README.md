# 🎯 No Hacking No CTF 2026

| **Dato** | **Información** |
|----------|-----------------|
| **Fecha** | 4-6 de julio de 2026 |
| **Modalidad** | Online |
| **Organiza** | No Hacking |
| **Equipo** | kr3s4l4 |
| **Puesto** | #112 |
| **Puntuación** | 879 pts |

## 🚩 Retos Resueltos

| Reto | Categoría | Puntos | Técnica | Writeup |
|------|-----------|--------|---------|---------|
| travel again | OSINT | 469 | Búsqueda visual + Google Earth | [Ver](./writeups/travel_again/README.md) |
| Kira-Notes | Forense | 100 | Carving forense (foremost) | [Ver](./writeups/Kira-Notes/README.md) |
| Final Boarding | OSINT | 100 | Metadatos EXIF + Flight tracking | [Ver](./writeups/Final_Boarding_481/README.md) |
| newbie-crypto | Crypto | 100 | Nonce Reuse (AES-CTR) | [Ver](./writeups/newbie-crypto/README.md) |
| Wotagei | OSINT | 100 | Búsqueda visual + Instagram | [Ver](./writeups/Wotagei/README.md) |
| Welcome | Bienvenida | 10 | Reto inicial | [Ver](./writeups/Welcome/README.md) |

## 🛠️ Técnicas Utilizadas

| Categoría | Técnicas |
|-----------|----------|
| **OSINT** | Google Lens, Google Earth, Flightera.net, Instagram, Búsqueda visual |
| **Forense** | foremost, sqlite3, 7z |
| **Crypto** | Nonce Reuse Attack, AES-CTR |

## 📊 Resumen por Categoría

| Categoría | Retos | Puntos |
|-----------|-------|--------|
| OSINT | 3 | 669 |
| Forense | 1 | 100 |
| Crypto | 1 | 100 |
| Bienvenida | 1 | 10 |
| **Total** | **6** | **879** |

## 📈 Progreso Temporal

| Día | Hora | Reto | Puntos |
|-----|------|------|--------|
| 4 jul | 2:02 | Welcome | 10 |
| 4 jul | 2:14 | newbie-crypto | 100 |
| 4 jul | 7:39 | Kira-Notes | 100 |
| 4 jul | 11:27 | Final Boarding | 100 |
| 5 jul | 10:10 | travel again | 469 |
| 6 jul | 0:13 | Wotagei | 100 |

## 💡 Hints Utilizados

| Reto | Hint |
|------|------|
| travel again | "Not the shooting spot — the object itself." |

## 🛠️ Herramientas Utilizadas

| Herramienta | Uso |
|-------------|-----|
| exiftool | Metadatos |
| sqlite3 | Bases de datos |
| foremost | Carving |
| 7z | Extracción ZIP |
| Google Lens | Búsqueda visual |
| Google Earth | Coordenadas |
| Flightera.net | Vuelos |
| Instagram | Búsqueda de perfiles |

## 📸 Capturas

![Ranking](./images/ranking.png)

## 📂 Writeups

- [travel again](./writeups/travel_again/README.md)
- [Kira-Notes](./writeups/Kira-Notes/README.md)
- [Final Boarding](./writeups/Final_Boarding_481/README.md)
- [newbie-crypto](./writeups/newbie-crypto/README.md)
- [Wotagei](./writeups/Wotagei/README.md)
- [Welcome](./writeups/Welcome/README.md)
